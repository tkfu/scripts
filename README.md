...
lines_data.csv - this is a raw output for every script in our dataset. We have 8,000 scripts. But we only have 2,000 films. That's because most of these screenplays are garbage. Sometimes they didn't parse well because they are scanned pdfs from the 1960s. Other times we mapped them to the incorrect film. Either way, I would not use this data and focus on the character_list csv (or only scripts used in the character_list csv)
